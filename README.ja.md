...
[v0.2.0 をダウンロード](https://github.com/yoonpooh/SwitchGPT/releases/tag/v0.2.0) · [最新リリース](https://github.com/yoonpooh/SwitchGPT/releases/latest)
...
1. [リリースページ](https://github.com/yoonpooh/SwitchGPT/releases/tag/v0.2.0)から `SwitchGPT-v0.2.0-macos-arm64.zip` と `SHA256SUMS.txt` をダウンロードします。
...
5. デスクトップアプリで Codex リクエストを送ります。実際のデスクトップモデル応答が完了すると、パネルで接続を確認できます。
...
- **次のリクエスト：** 新しいモデルリクエストに使うアカウントです。カードで変更します。
- **直近の処理：** 最後にモデル応答を実際に完了したアカウントです。カードの選択だけではこの結果は変わりません。
- **デスクトップログイン：** デスクトップのアカウントを下部に別途表示します。
- **自動切り替え：** `⋯` で初期状態から有効です。自動切り替え直後にはバッジが短時間表示されます。
...
リセットクレジットと有効期限は**表示専用**で、SwitchGPT は消費しません。日付は Mac のタイムゾーンと表示言語に従います。Mac の優先言語を変えたらアプリを開き直してください。未対応の言語は英語、中国語の地域・文字体系の違いは簡体字で表示します。

## 自動切り替え

1. 選択アカウントのいずれかの利用枠が尽きると、最近の取得で残量が確認できた最初のアカウントをリスト順に選びます。
...
`⋯` で自動切り替えを無効にすると、選択アカウントを使い続け、その上限エラーをそのまま返します。カードの順番が選択の優先順位になり、利用枠はアカウントごとに独立しています。
...
スクリプトはビルドを実行する Mac のアーキテクチャ向けにビルドします。公開済みの v0.2.0 は Apple シリコン向けです。
