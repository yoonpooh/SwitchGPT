...
`⋯` → Jev 설정에서 API 키를 저장한 뒤 **모델 자동 선택**과 **추론 수준 자동 선택**을 각각 켤 수 있습니다. 새 설정의 기본값은 둘 다 꺼짐이며, 기존 **계정 자동 전환**과 별도로 동작합니다. 이전 버전에서 모델 자동 선택을 켰다면 모델과 추론 수준을 함께 조절하던 동작을 보존합니다. 키는 macOS 키체인에 저장됩니다.

- 방향을 판단할 수 있는 Luna/Sol/Astra 요청을 대상으로 합니다. 구형 Codex·다른 모델은 외부 분류 없이 유지합니다.
- 기준 모델과 추론 수준은 Codex가 요청에 담아 보낸 값입니다. 꺼져 있거나 판단이 불확실하거나 Jev 호출이 실패하면 원래 값을 유지합니다.
- 모델과 추론 수준은 한 번의 Jev 요청에서 별도로 판단합니다. 모델만 켜면 기존 추론 수준을, 추론 수준만 켜면 기존 모델을 보존합니다. 지원하지 않는 최종 조합은 적용하지 않으며, 사용 중인 계정의 모델 접근 권한을 추가하지 않습니다.
- 자동 effort 후보 medium/high/max는 세 모델 모두에 적용할 수 있습니다. 기존 low 요청도 기준을 충족하면 상향할 수 있으며, xhigh/ultra 또는 생략된 effort는 보존합니다.
...
아래 조합은 작업별 선택 의도의 예시입니다. 한 항목만 자동 선택하면 다른 항목은 요청의 원래 값을 유지합니다.

분류는 작업의 크기뿐 아니라 **해결 방법이 정해져 있는지, 원인 조사가 필요한지, 영향 범위가 어디까지인지**를 구분합니다.

| 조합 | 선택 기준 |
| --- | --- |
| Luna medium | 번역·조회·요약·정해진 도구 작업과 입력/출력이 명확한 작은 구현. 테스트나 반복 횟수만으로 effort를 올리지 않음 |
| Luna max | 설계는 정해져 있지만 복잡한 알고리즘·수학적 정합성 또는 만료·순서·취소 등 상호작용 제약을 지켜야 하는 구현 |
| Sol medium | 코드·앱·설정·자료에서 원인을 찾거나 방법을 비교·선택하는 일반 조사 |
| Sol high | 경쟁 상태·복잡한 상태 전이·데이터 보존 이전 등 범위가 정해진 어려운 분석 |
| Astra medium | 여러 시스템에 걸친 설계·통합·단계별 계획 |
| Astra high | 큰 불확실성과 데이터 정합성 위험이 있는 시스템 전반의 분석 |

각 판단의 `confidence`를 별도로 검사합니다. effort 상향 기준은 0.65이며, effort 하향과 모델 변경 기준은 0.8입니다. 기준 미달인 항목은 원래 값을 유지합니다. 이 값은 후보 확률 분포의 집중도이며, 분류 정확도나 실제 작업 성공 확률이 아닙니다. 직전 사용자 요청으로 짧은 후속 지시의 작업을 식별할 수 있으면 함께 판단합니다. 맥락이 부족하거나 모델·effort·오케스트레이터를 명시한 요청, 기존 테스트·보호 파일의 수정을 엄격히 금지한 요청, 삭제·초기화·운영 배포·공개 발행·메시지 전송·접근 권한 변경을 실행하는 요청은 원래 설정을 유지하도록 분류 지침에 포함합니다. 명확한 테스트·파일 보존 금지 문구는 로컬 검사에서도 전환을 생략합니다. 이 검사는 제한된 표현만 인식하며, 기준 모델 자체의 지시 준수나 파일 접근 권한을 보장하지 않습니다. 분류 지침은 별도의 권한 검사나 실행 안전장치를 대체하지 않습니다.

라우터는 모델 선택을 보조하며 작업을 분해하거나 결과의 정확성을 검증하는 오케스트레이터가 아닙니다. 전환을 안전하게 식별할 수 없는 요청과 지원하지 않는 요청 형식은 원래 설정을 유지합니다. 실제 Jev 품질·지연 및 Codex 데스크톱 호환성은 사용 환경에서 별도로 확인해야 합니다.

effort 적용 보완과 판단 로그는 [effort 보완 검증](docs/effort-routing-fix-2026-09-21.md)을 참고하세요. 이번 분리 정책과 검증 범위는 [독립 라우팅 검증](docs/jev-independent-routing-2026-09-21.md)을 참고하세요. 이전 결합 정책의 평가 방법과 결과는 [분류 검증](docs/jev-router-evaluation-2026-09-20.md), [모델 실행 비교](docs/jev-router-execution-evaluation-2026-09-20.md), [effort 최적화와 저장소 작업 검증](docs/jev-router-optimization-2026-09-20.md)을 참고하세요.
