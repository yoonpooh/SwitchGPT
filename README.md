...
A lightweight macOS menu bar app for switching between the ChatGPT accounts you use with **Codex desktop**.
...
- Add accounts through browser sign-in without replacing the current Codex session.
...
- Switch accounts after confirmation; Codex and its local background server restart as part of the switch.
...
- The default file-based Codex credential store at `~/.codex/auth.json`.

SwitchGPT locates the desktop application by bundle identifier `com.openai.codex`. It does not switch accounts in the browser or the separate consumer ChatGPT app. It uses the CLI bundled with Codex; a separate CLI installation is not required.
...
3. **Switch:** finish any active Codex work, click another account's card, and confirm the restart. Clicking the current account does not switch it.
...
Before switching, the app saves the current credentials and replaces the active credential file using a temporary file with `0600` permissions. If restarting Codex fails, it attempts to restore the previous credentials and reports recovery failures.

Usage information is retrieved directly from OpenAI's private `chatgpt.com/backend-api/wham/` endpoints. There is no SwitchGPT backend. These endpoints and Codex's local server behavior can change, and expired credentials may require signing in again. A successful local restart alone does not prove that the server accepted the new session.
...
- Switching may affect CLI sessions that share Codex's local server. Finish active work before switching.
- Missing usage or expiration data is shown as unavailable rather than as zero.
- Building successfully does not establish compatibility with every Codex desktop version or every supported macOS version.
...
Sources/CodexAccountSwitch/
  ...
  Services/                     Login, Keychain, usage, and Codex session handling
