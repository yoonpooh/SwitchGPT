...
[下载 v0.2.8](https://github.com/yoonpooh/SwitchGPT/releases/tag/v0.2.8) · [最新版本](https://github.com/yoonpooh/SwitchGPT/releases/latest)

## 0.2.8 的主要更新

- **修复桌面登录显示：** 即使桌面使用与已保存账户不同的登录方式（例如通过 Apple 登录），也会识别为同一账户，并显示账户名称和头像，而不是“当前登录账号”。
...
1. 从[发布页面](https://github.com/yoonpooh/SwitchGPT/releases/tag/v0.2.8)下载 `SwitchGPT-v0.2.8-macos-arm64.zip` 和 `SHA256SUMS.txt`。
...
脚本会为运行构建的 Mac 的架构生成应用。已发布的 v0.2.8 文件是 Apple 芯片构建。
